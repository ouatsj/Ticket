    ...
    <div class="row">
        <p class="mt-0 mb-2 ml-4">
            ...
            <a href="<?//= site_url("caisses/{$this->session->company->ekey}". "/cais/".
                ...
                //"/depensecourrier_adjoint/".$bus_stop->idsousgare.'/'. mdate("%d/%m/%Y", now('UTC'))); ?>" class="btn btn-space btn-secondary">
                    
                <i class="fas fa-arrow-circle-down text-info"></i>&nbsp;DEPENSES COURRIERS&nbsp;
            </a>
            <? if ($this->session->agent->userole === '1' OR $this->session->agent->userole === '2' OR $this->session->agent->userole === '5' OR $this->session->agent->userole === '16'): ?>
                <button class="btn btn-space btn-secondary addtridepense md-trigger" data-cle_compagnie="<?= $this->session->company->ekey; ?>"
                    ...
                    <i class="fas fa-edit text-warning"></i>&nbsp;HISTORIQUE DEPENSES&nbsp;
                </button>
            <?endif;?>
        </p>
    </div>
...
<div class="modal-container colored-header colored-header-success custom-width modal-effect-7"
            id="form-formtridepens" style="perspective: none;">
    ...
    <div class="modal-content">
        ...
        <?= form_open("Depenses/triadjointdate/{$this->session->company->ekey}/{$caisseident->gexp_caiss}/{$caisseident->id_caiss}",
                    array('class' => 'modal-body form')); ?>
        <div class="form-group row">
            <input class="form-control form-control-sm" type="hidden" name="gareconnect" value="<?=$bus_stop->idengare;?>">
            <input class="form-control form-control-sm" type="hidden" name="sousgareconnect" value="<?=$bus_stop->idsousgare;?>">
