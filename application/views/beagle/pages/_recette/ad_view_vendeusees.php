...
if ($escale_nom !== '') {
    ...
    $lien_retour_recette = site_url(
        "caisses/{$this->session->company->ekey}/cais/" . $gexp_esc . '/'
        . (!empty($caisseident->id_caiss) ? $caisseident->id_caiss : 0) . '/' . $conex->roleattribut
        . '/recette_adjoint/' . $bus_stop->idsousgare . '/' . mdate('%d/%m/%Y', now('UTC'))
    );
...
<!--End of file: ad_view_vendeusees.php-->
<!--File location: application/views/beagle/pages/_recette/ad_view_vendeusees.php-->                            