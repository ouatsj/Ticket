...
<div class="col-12">
    <div class="card card-table">
        <div class="card-header">
            ...
            <div class="title">bagages avec ticket</div>
        </div>
        <div class="card-body">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <? if ($this->session->agent->userole === '1' OR $this->session->agent->userole === '10' OR $this->session->agent->userole === '12'): ?>
                                ...
                                href="<?= site_url('Historique_Passagers/bagsave/'.$this->session->company->ekey.'/'.$item->id_bagage.'/'.$bus_stop->idengare.'/'.$conex->roleattribut.'/'.$bus_stop->idsousgare); ?>">
                        ...
                        <td>
                            ...
                            <? if ($this->session->agent->userole === '1' OR $this->session->agent->userole === '6'): ?>
                                <a class="icon" title="epson"
                                href="<?= site_url('Historique_Passagers/bagsaveguich/'.$this->session->company->ekey.'/'.$item->id_bagage.'/'.$bus_stop->idengare.'/'.$conex->roleattribut.'/'.$bus_stop->idsousgare); ?>">
                                <i class="fas fa-print"></i>
                            <? endif; ?>&nbsp;&nbsp;&nbsp;
                            <? if ($this->session->agent->userole === '1'): ?>
                            <a class="icon" title="ANNULER RECU"
                                href="<?= site_url('Confirmation/annule/'.$this->session->company->ekey.'/'.$conex->roleattribut.'/'.$item->id_bagage.'/'.$bus_stop->idengare.'/'.$bus_stop->idsousgare.'/'.$item->annulebag); ?>">
                            ...
                            <div
                                ...
                                <div class="modal-content">
                                    ...
                                    <?= form_open('Confirmation/updateprix/' . $this->session->company->ekey .'/'.$conex->roleattribut.'/'.$bus_stop->idengare.'/'.$bus_stop->idsousgare.'/'.$item->id_bagage, array('class' => 'modal-body form')); ?>
...
<div class="modal-container colored-header colored-header-success custom-width modal-effect-7"
        id="tribagage" style="perspective: none;">
    <div class="modal-content">
        <div class="modal-header modal-header-colored">
            <h3 class="modal-title" id="">TRI BAGAGES</h3>
            <button class="close modal-close" type="button"
                data-dismiss="modal" aria-hidden="true"><span
        ...
        <?= form_open("confirmation/voirbagaggetri/{$this->session->company->ekey}/{$conex->roleattribut}/{$bus_stop->idengare}/{$bus_stop->idsousgare}", array('class' => 'modal-body form')); ?>
