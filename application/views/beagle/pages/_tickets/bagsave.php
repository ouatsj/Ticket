                ...
                <div class="tab-pane active show" id="icon1" role="tabpanel">
                    ...
                    <table>
                        <? $ckey = $this->session->company->ekey;
                          ...
                          $key = mdate("%Y-%m-%d", now());
                          $dtoday = $key.' à '.$dats;
                          $dat = explode("-", $itembgs->date_create);
                          $day = $dat[2]. '-'. $dat[1]. '-' .$dat[0];
                           $cdebg = explode("-", $itembgs->id_bagage);
                          $encd = $cdebg[0];
                        ?>
                        <tr><td style="font-size: 70px; width: 40%;"> <img src="<?echo site_url($itembgs->logo);?>" width="850" height="350"></td></tr>
                        <tr><td style="font-size: 65px;"><b>Reçu N° : <?=$encd; ?></b></td></tr>
                        <tr><td style="font-size: 60px;"><?= $itembgs->nom_gaep; ?> <?= "{$bus_stop->nomsousgare}"; ?>-<?= $itembgs->nom_gadest; ?> <?= $itembgs->quartarr_bg; ?></td></tr>
                        <tr><td style="font-size: 65px;">Contact exp:<?= $itembgs->contactexpedi; ?> <?= $itembgs->genrebagage; ?></td></tr>
                        <tr><td style="font-size: 65px;">Client:<?= $itembgs->nom_client; ?> <?= $itembgs->prenom_client; ?></td></tr>

                        <tr><td style="font-size: 65px;">Nombre bagage&nbsp;:&nbsp;<b><?= $itembgs->nombrebagage; ?>&nbsp;(<?=$itembgs->typebagages; ?>)</b></td></tr>
                        <tr><td style="font-size: 60px;">Contenu:<?= $itembgs->contenubagage; ?></td></tr>
                        
                        <tr><td style="font-size: 65px;">Valeur:<?= $itembgs->valeurbagage; ?>&nbsp;FCFA</td></tr>
                       
                        <tr><td style="font-size: 65px;">Prix:<b>&nbsp;&nbsp;&nbsp;<?= number_format($itembgs->prix_bagage, 0, '', ' '); ?> &nbsp;FCFA </td></tr>
                        <tr><td style="font-size: 65px;">Facturer le :<b><?= $day; ?></td></tr>
                        <tr><td style="font-size: 40px;">emis : <?= $dtoday; ?> &nbsp;par <?= $itembgs->username; ?> code <?= $itembgs->codebag; ?></td></tr>
                        <tr><td style="font-size: 40px;"><?= $itembgs->nom_compagnie;?> décline toute responsabilité en cas de</td></tr>
                        ...
                        <tr><td style="font-size: 50px;">BON VOYAGE AVEC <?= $itembgs->nom_compagnie;?></td></tr>
                        </table>
                    </body>
                </div>
            </div>
...
<!--End of file: bagsave.php-->
<!--File location: application/views/beagle/pages/_tickets/bagsave.php-->